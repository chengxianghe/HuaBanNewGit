...
{
    //\uFE0F
    if ([text rangeOfString:@"\uFE0F"].length > 0) {
        NSMutableString *s = [[NSMutableString alloc] initWithString:text];
        while ([s rangeOfString:@"\uFE0F"].length > 0) {
            [s deleteCharactersInRange:[s rangeOfString:@"\uFE0F"]];
        }
        text = s;
    }
...
+ (NSDictionary *)emojiAliases {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
//        NSString *path = [[NSBundle mainBundle] pathForResource:@"SystemEmojize" ofType:@"plist"];
